...
export function AlarmList({ selected, onSelect }: AlarmListProps) {
    ...
    return (
        <div className="space-y-3">
            {/* Column Headers */}
            <div className="sticky top-0 bg-card/95 backdrop-blur-sm z-10 flex items-center justify-between px-4 py-3 text-xs font-bold text-muted/70 uppercase tracking-wider border-b border-line/50">
            ...
            <div className="max-h-[400px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                {items.map(item => (
                    <div
                        key={item.id}
                        className={`group flex items-center justify-between px-4 py-4 rounded-xl border transition-all duration-200 ${selected.has(item.id)
                    ...
                    >
                        ...
                        {/* Time Display */}
                        <div className={`w-28 text-2xl font-bold tabular-nums transition-colors ${item.notify_status === 'SENT' ? 'text-green-400' :
                        ...
                        {/* Action Buttons */}
                        <div className="w-36 flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            {item.audioId && (
                                <button
                                    onClick={() => handlePlay(item)}
                                    className="p-2 hover:bg-primary/20 rounded-lg text-primary transition-all hover:scale-110"
                            ...
                            <button
                                ...
                                className="p-2 hover:bg-danger/20 rounded-lg text-danger transition-all hover:scale-110"
                                title="Delete alarm"
